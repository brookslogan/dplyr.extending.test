...
test_that("to_keyed_tibble1 works as expected", {
  # Successful output is what was expected:
  expect_equal(to_new_keyed_tibble1(tibble(x = 1:5), "x"),
  ...
  expect_equal(to_new_keyed_tibble1(intermediate_result, "x"),
               `class<-`(new_keyed_tibble1(tibble(x = 1:5), "x"),
                         c("keyed_tibble1", "grouped_df", "tbl_df", "tbl", "data.frame")))

  expect_equal(to_new_keyed_tibble1(new_keyed_tibble1(tibble(x = 1:5, y = 1:5), "x"), "y"),
               new_keyed_tibble1(tibble(x = 1:5, y = 1:5), "y"))

  # Type validation is handled:
  expect_error(to_new_keyed_tibble1(data.frame(x = 1:5), "x"))
  expect_error(to_new_keyed_tibble1(tibble(x = 1:5), 12345))

  # Non-type validation isn't handled in this constructor:
  expect_no_error(to_new_keyed_tibble1(tibble(x = 1:5), "y"))
  expect_no_error(to_new_keyed_tibble1(tibble(x = c(1,1,2,3)), "x"))
})
